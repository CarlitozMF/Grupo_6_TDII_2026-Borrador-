...
const uint32_t tiempos[] = {100, 250, 500, 1000};

const uint8_t cant_intervalos = sizeof (tiempos)/ sizeof(tiempos[0]);
static uint8_t indice_tiempo = 0;
static bool btn_presionado = false;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  /* 1. Capa de Entrada (Botón) */
	      if (HAL_GPIO_ReadPin(USER_BTN_GPIO_Port, USER_BTN_Pin) == GPIO_PIN_SET) {
	          if (!btn_presionado) {
	              indice_tiempo++;
	              if (indice_tiempo >= cant_intervalos) indice_tiempo = 0;
	              btn_presionado = true;
	          }
	      } else {
	      ...
	      /* 2. Capa de Procesamiento (Parpadeo) */
	      // Toggle simultáneo de los 3 LEDs
	      for(uint8_t i = 0; i < cant_leds; i++) {
	          HAL_GPIO_TogglePin(leds_array[i].port, leds_array[i].pin);
	      }

	      // Espera según la frecuencia seleccionada
	      HAL_Delay(tiempos[indice_tiempo]);
  }
  /* USER CODE END 3 */
}
