...
// Arreglo de intervalos configurados para el parpadeo
static const uint32_t tabla_tiempos[] = {100, 250, 500, 1000};
static const uint8_t cant_intervalos = sizeof(tabla_tiempos) / sizeof(tabla_tiempos[0]);

// Variables de control de índice en español
static uint8_t indice_tiempo_actual = 0;
...
int main(void)
{
	...
	while (1)
	{
		...
		/* 1. Capa de Entrada (Botón genérico) */
		// Evalúa el flanco de presión de forma directa
		if (BOTON_DetectarFlancoPresionado(INDICE_BTN_USER)) {
			indice_tiempo_actual++;
			if (indice_tiempo_actual >= cant_intervalos) {
				indice_tiempo_actual = 0; // Reinicio circular de velocidad
			}
		}

		/* 2. Capa de Procesamiento (Parpadeo simultáneo abstracto) */
		// Toggle dinámico de todos los LEDs registrados en el sistema
		for (uint8_t i = 0; i < LED_GetCount(); i++) {
			LED_Toggle(i);
		}

		/* 3. Base de tiempo bloqueante configurable */
		// Toma el intervalo de retraso dinámicamente según el índice actual
		TIME_DelayMS(tabla_tiempos[indice_tiempo_actual]);
	}
	/* USER CODE END 3 */
}
